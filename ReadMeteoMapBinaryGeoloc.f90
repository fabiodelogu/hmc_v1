...
subroutine ReadMeteoMapBinaryGeoloc(xstring,iRows,iCols)

implicit none
include 'DeclarationH.f90'

CHARACTER*12  xstring,ext
CHARACTER*500  file,rainfile,temperaturefile,radiationfile,windfile,umidityfile,maskfile,laifile,line
CHARACTER*200 sCheck
INTEGER i,j,ios,is,iTLength,iiw,iw
INTEGER iRows,iCols,iFhum
INTEGER*4 iLStr,iPathLenght,temp(iRowsMeteo,iColsMeteo)
REAL*8 a2dMaskSnow(iRowsMeteo,iColsMeteo),a2dMeteo(iRowsMeteo,iColsMeteo),dTW
...
open(22,file='Rain_'//xstring,status='old',form='unformatted', &
          access='direct',recl=iRowsMeteo*4,iostat=ios)
do j=1,iColsMeteo
   read(22,rec=j)(temp(iRowsMeteo-i+1,j),i=1,iRowsMeteo)
end do

a2dMeteo=real(temp)/dRescFct
temp=0
close(22)
...
!Geolocation on Dem grid
CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dRain)
...
if(1.eq.1)then
	...
	open(22,file='SnowMask_'//xstring,status='old',form='unformatted', &
			  access='direct',recl=iRowsMeteo*4,iostat=ios,err=224)
	do j=1,iColsMeteo
	   read(22,rec=j)(temp(iRowsMeteo-i+1,j),i=1,iRowsMeteo)
	end do
	close(22)
	a2dMeteo=real(temp)/dRescFct
	!Geolocation on Dem grid
    CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dMaskSnow)
...
!Geolocation on Dem grid
CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dTemp)
...
!open(22,file='Rad_'//xstring,status='old',form='unformatted', &
          access='direct',recl=iRowsMeteo*4,iostat=ios,err=222)
do j=1,iColsMeteo
   read(22,rec=j)(temp(iRowsMeteo-i+1,j),i=1,iRowsMeteo)
end do
close(22)
...
!Geolocation on Dem grid
CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dK)
...
!Geolocation on Dem grid
CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dW)
...
open(22,file=umidityfile(1:iTLength)//'.gz',status='old',form='unformatted', &
          access='direct',recl=iRowsMeteo*4,iostat=ios,err=225)
close(22)
if(1.eq.2)then !Prova con altro nome (uso iPathLenght perchè Umidità è l'ultima variabile)
...
close(22)
a2dMeteo=real(temp)/(dRescFct*100) !Umidità nei dati è in percentuale
...
!Geolocation on Dem grid
CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dUm)
...
if(ora.eq.0)THEN
	if(giorno.eq.1.or.giorno.eq.15)THEN
		...
		open(22,file=laifile(1:iTLength),status='old',form='unformatted', &
				  access='direct',recl=iRowsMeteo*4,iostat=ios,err=223)
		close(22)
	...
	!	call system(line)
		...
		!Geolocation on Dem grid
		CALL Geoloc_meteo_file_GRID(iRows,iCols,iRowsMeteo,iColsMeteo,a2dMeteo,a2dLai)
