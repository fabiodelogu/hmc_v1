...
SUBROUTINE DeterminaData_stepParamHour(xstring,xstring_up,iDtDate,iDmeteo)

IMPLICIT NONE


INTEGER*4 anno,iBanno,mese,giorno,ora,flag,min,iDtDate,iDiff,i,iDmeteo
...
IF(giorno.EQ.30.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff.AND.flag.EQ.0)THEN
...
IF(giorno.EQ.31.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff.AND.flag.EQ.0)THEN
...
IF(mese.EQ.2.AND.flag.EQ.0)THEN
	IF(giorno.EQ.29.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff.AND.flag.EQ.0)THEN
...
IF(mese.EQ.2.AND.flag.EQ.0)THEN
	 IF(giorno.EQ.28.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff.AND.flag.EQ.0)THEN
...
IF(flag.EQ.1.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff)THEN
	min=min+iDtDate-int(iDtDate/60)*60
    ora=int(iDtDate/(60*iDmeteo))
ENDIF

IF(flag.EQ.0.AND.(ora+int(iDtDate/60)).GT.23.AND.min.GE.iDiff)THEN
	giorno=giorno+1
	min=min+iDtDate-int(iDtDate/60)*60
    ora=int(iDtDate/(60*iDmeteo))
	flag=1
