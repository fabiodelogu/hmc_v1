...
REAL*8,  ALLOCATABLE, TARGET :: a2dAAreeWT(:,:)
REAL*8,  ALLOCATABLE, TARGET :: a2dAC1(:,:),a2dAF2(:,:)
REAL*8,  ALLOCATABLE, TARGET :: a2dAEvtCum(:,:)
REAL*8,  ALLOCATABLE, TARGET :: a2dACt(:,:),a2dACf(:,:),a2dACappaC(:,:),a2dACappaV(:,:)
...
REAL*8,  ALLOCATABLE, TARGET :: a1dAQmaxRil(:),a1dAQminEco(:)
...
! Moltiplicatore piogge
REAL*8,  ALLOCATABLE, TARGET :: a2dARmul(:,:)
REAL*8,  ALLOCATABLE, TARGET :: a2dACoeffResol(:,:)
!-----------------------------------------------------------------------------------
...
CHARACTER*12  xstring,xstring_up,xstring_do,sDataIni,sVariabile,xDate
...
real*8 dSt1,dSt2,dTime_spent !Variabili per calcolo dei tempi
...
dKsatRatio=1
dETDayMean=0
dTmaxFor=-9999; !Max routing time after observation in hours
!nArg= NARGS( )
!write(*,*)nArg
...
if(dSlopeMax<0.01)then
	dSlopeMax=0.01 !Valore standard
endif
!***********************************************************************************
!***********************************************************************************
! VERSION
write(*,*)'***************************'
write(*,*)'version: 1.0 '
write(*,*)'Author of modify: Silvestro F.'
write(*,*)'date last modify: 12/03/2015'
write(*,*)'***************************'
...
ALLOCATE(a2dAAreeWT(iRows,iCols))
ALLOCATE(a2dAC1(iRows,iCols),a2dAF2(iRows,iCols))
ALLOCATE(a2dACt(iRows,iCols),a2dACf(iRows,iCols),a2dACappaC(iRows,iCols),a2dACappaV(iRows,iCols))

ALLOCATE(a2dAEvtCum(iRows,iCols))
...
a2dCt				=> a2dACt
a2dCf				=> a2dACf
a2dCappaC			=> a2dACappaC
a2dCappaV			=> a2dACappaV

a2dCt=dCt !Intialize with constant value from command line
a2dCf=dCf !Intialize with constant value from command line

a2dEvtCum			=> a2dAEvtCum
a2dVwt				=> a2dAVwt
...
!Inizializzo la Tsuolo e la Evt
a2dTs=0.0
a2dEvapot=0.0
...
a2dCappaC=dCappaC !Intialize with constant value from command line
a2dCappaV=dCappaV !Intialize with constant value from command line
a2dCt=dCt !Intialize with constant value from command line
a2dCf=dCf !Intialize with constant value from command line
CALL ReadLandData(iRows,iCols)

!Change units the surface flow parameters in the one used in the code
where(a2dDEM.gt.0.0.and.a2dCappaV.lt.0.05)
    a2dCappaV = a2dCappaV*3600 !from 1/s to 1/h
    a2dCappaC = a2dCappaC*(3600*1000)/(sqrt(dCelLat*dCelLon)*1000**(dBc+1)) !from m^0.5/s to 1/(h*mm^0.5)
endwhere 

!-----------------------------------------------------------------------------------
!   Call to Subroutine CoeffResolutionMap
!   coeff for spatial resolution
!-----------------------------------------------------------------------------------
ALLOCATE(a2dACoeffResol(iRows,iCols))
a2dCoeffResol => a2dACoeffResol
a2dCoeffResol=0.0
CALL CoeffResolutionMap(iRows,iCols)
!-----------------------------------------------------------------------------------
...
!Condition to add in the case we need to avoid too long simulation after th end of
!meteo input data.
IF(dTc.gt.840)dTc=840 !No more than 5 weeks
IF(dTmaxFor.gt.0.0)dTc=dTmaxFor !Fixed by the user

dCTimeMax=dTc*3600 !dTc in seconds after the end of the rain
...
WRITE(*,'(A11,1x,f12.1,1x,A7)')'AREA DOMAIN',dBasinArea*dCelLat*dCelLon/1000000,'[km^2]'
...
ALLOCATE(a1dA_Level(iNdam,600),a1dA_Volume(iNdam,600),a1dAHmax(iNdam),adAL(iNdam),a1dACoefDighe(iNdam))
...
ALLOCATE(a1dAQmaxRil(iNril),a1dAQminEco(iNprese))
...
a1dQmaxRil	=> a1dAQmaxRil
a1dQminEco => a1dAQminEco
...
!File dove vengono scritti i V medi a scala di bacino (solo sperimentale)
OPEN(unit=312,file=sBasin(1:iNameLenght)//'_Vmean.out') 
!-----------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------  
			!Manages the possibility of having meteo data with frequency different
			!from data resolution
			if(dFreqMeteo.ne.dMinutes/60.and.dFreqMeteo.gt.1)then										
				read(xstring(9:10),*)dHourM
			    if(int(dHourM/dFreqMeteo).ne.dHourM/dFreqMeteo)then
					dHourM=dFreqMeteo+int(dHourM/dFreqMeteo)*dFreqMeteo-dHourM
					CALL DeterminaData_stepParamHour(xstring,xDate,int(dHourM)*60,int(dFreqMeteo))
			...
			ELSE
				...
				IF(iFlagGrid.eq.1)THEN	
					...
					CALL ReadMeteoMapBinaryGeoloc(xDate,iRows,iCols)
				ELSE
					!Meteo and Dem grids equal
					CALL ReadMeteoMapBinary(xDate,iRows,iCols)
				ENDIF
				!Check meteo Matrix values
				CALL CheckMeteoMapBinary(xstring)
				!Delete File Meteo for SuperMUC
				if(1.eq.0)then
					CALL DeleteMeteoMapBinary(xDate,iRows,iCols)
				endif   
			    ! Updating Date
				CALL DeterminaData_stepParam(xstring,xstring_up,int(dMinutes))
			...
			!write(*,*)xstring,' ',xDate
			!Multiply for correction factor
			a2dRain=a2dRain*a2dRmul
...
!-----------------------------------------------------------------------------------				    			                
			...
			WRITE(311,'(f6.2)') dLST !Scrive la Ts media
			dLST=-9999
!-----------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------
			...
			CALL evapotranspiration_FR_matrix(iRows,iCols,d,dEvTTot,xstring_do)
					dETDayMean=0*dETDayMean+dEvTTot(1)/dBasinArea
					
			write(*,*)xstring_do,' EvtMean= ',dEvTTot(1)/dBasinArea
				
			IF(h.eq.dDayStep)THEN
			    iL = iLStr(sPathLandData)
			    CALL FDATE(whatimeisit)
				write(*,*)'**************'
				write(*,*)
				write(*,*)whatimeisit,' ',xstring_do,' ',sPathLandData(1:iL)
				WRITE(*,*)'Sim step=',m
				write(*,*)'**************'
...
!-----------------------------------------------------------------------------------
			...
			ENDDO		
			WRITE(312,'(f6.2)') dVtot/dBasinArea !Scrive il V medio
!-----------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------
!	Saving State variable and Q matrix
!	Calcola coeff serbatoio lineare equivalente in caso di svaso dagli organi di scarico
!	delle dighe
!----------------------------------------------------------------------------------- 
...
600 FORMAT (f8.1,1000(f11.2,1x))
!600 FORMAT (f8.1,1000(f9.4,1x))
...
DEALLOCATE(a2dAAreeWT)
DEALLOCATE(a2dAVwt,a2dADeepFlow,a2dAVwtMax,a2dABeta)

DEALLOCATE(a2dAC1,a2dAF2)
DEALLOCATE(a2dACt,a2dACf,a2dACappaC,a2dACappaV)

DEALLOCATE(a2dAEvtCum)
...
DEALLOCATE (a2dQSotT,a2dQT,a2dEvTMat)
...
DEALLOCATE(a1dAQmaxRil,a1dAQminEco)
...
DEALLOCATE(a2dARmul)
DEALLOCATE(a2dACoeffResol)
DEALLOCATE(a2iAYLat,a2iAXLon)

DEALLOCATE(a2dANature,a2dASnowFall,a2dAExp,a2dAAge)
DEALLOCATE(a2dASWE,a2dAAlbedo,a2dARoS0,a2dARoS,a2dAMeanDayTemp)
DEALLOCATE(a2dAMeltingDayCum,a2dAMelting,a2dAsMc,a2dAsMcGlac)
