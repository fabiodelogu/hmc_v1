...
subroutine Read3DMapBinary(iRows,iCols,iTime,sPathDati,sNameOut,a2dMap,iDivide,iTgzip)
...
INTEGER i,j,k,ios,is,iLength,iLo
...
INTEGER*4 a2iMap(iRows,iCols,iTime),iDivide
...
open(22,file=sFileName(1:iPathLenght)//'.gz',form='unformatted', &
          access='direct',status='old',recl=iRows*4,iostat=ios,err=324)
close(22)
...
do k=1,iTime
	do j=1,iCols
	   read(22,rec=j)(a2iMap(iRows-i+1,j,k),i=1,iRows)
	end do
enddo
close(22)

a2dMap=real(a2iMap)/real(iDivide)
