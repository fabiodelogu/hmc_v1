...
!Pongo dDth=dDintegr, ma potrei differenziare canale e versante	
	...
	!Pongo a 0 il V(t) nelle celle corrispondenti alle dighe
	DO iD=1,iNdam
		i=a2dXYDam(iD,2)
		j=a2dXYDam(iD,1)
		a2dV(i,j)=0.0
...
!-------
				ELSE
!	Riempimento della cella successiva
					!dRate=a2dV(i,j)/a2dS(i,j)*(dcos(a2dAlpha(i,j))**2-dsin(a2dAlpha(i,j))**2)+dsin(a2dAlpha(i,j))**2
...
!	Volume perso verso gli strati profondi
	...
	!Riempio la diga con il deflusso ipodermico della cella diga
	DO iD=1,iNdam
		i=a2dXYDam(iD,2)
		j=a2dXYDam(iD,1)
